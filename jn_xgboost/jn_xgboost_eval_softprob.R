if(xg_nfold != FALSE) {
  ...
  colnames(xg_settings) = c("Age", "Learning_Rate", "Max_Tree_Depth", "Subsampling_Cells",
  ...
  for (j in 1:nrow(xg_run_results)) {
    ...
    xg_tmp_vp = cbind.data.frame("Age" = rep(as.factor(xg_labels_val), length(unique(xg_labels_val))),
                                 "Probability" = c(xg_tmp_pred),
                                 "Prediction" = as.factor(c(sapply(sort(unique(xg_labels_val)),
                                                                   function(x) rep(x,length(xg_labels_val))))))
    
    xg_tmp_plot_vp = ggplot(xg_tmp_vp, aes(x = Age, y = Probability)) +
