if(xg_nfold != FALSE) {
  ...
  for (j in 1:nrow(xg_run_results)) {
    ...
    xg_tmp_vp = cbind.data.frame("Predicted" = xg_tmp_pred,
                                 "Age" = as.factor(round(xg_tmp_labels_val,2)))
    
    xg_tmp_plot_vp = ggplot() +
      geom_violin(data = xg_tmp_vp, aes(x = Age, y = Predicted, fill = Age)) +
      scale_fill_brewer(direction = -1) +
      geom_boxplot(data = xg_tmp_vp, aes(x = Age, y = Predicted, fill = Age), width=0.1, outlier.shape = NA, fill = "white") +
      theme_classic() +
      theme(text = element_text(size=9)) +
